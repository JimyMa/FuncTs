...
You can also run the pass step by step and print the mid result (Convert to TensorSSA -> Remove Update -> DCE & CSE)
...
- The fusion and code generation process of view [tensorexprs](./test/test_immut_tensorexpr.py) are to slow since the Recursive-Descent Visiting in NNC is not efficient enough.

We implement a symbolic [shape inference](./fait/passes/refine_types.cpp) module and a [PyTorch NNC extension](./fait/fuser) to fill the gap. The API usage is exampled as follow:
