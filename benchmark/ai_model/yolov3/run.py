...
# fait
cls_scores: [torch.Size([1, 486, 20, 20]), torch.Size([1, 486, 10, 10]), torch.Size(
    [1, 486, 5, 5]), torch.Size([1, 486, 3, 3]), torch.Size([1, 486, 2, 2]), torch.Size([1, 486, 1, 1])]
bbox_preds: [torch.Size([1, 24, 20, 20]), torch.Size([1, 24, 10, 10]), torch.Size(
    [1, 24, 5, 5]), torch.Size([1, 24, 3, 3]), torch.Size([1, 24, 2, 2]), torch.Size([1, 24, 1, 1])]
type_hint = [torch.TupleType([torch.TensorType.get().with_dtype(torch.float32).with_sizes([arguments.bs, 255, 10, 10]).with_device(torch.device("cuda")),
                              torch.TensorType.get().with_dtype(torch.float32).with_sizes(
                                  [arguments.bs, 255, 20, 20]).with_device(torch.device("cuda")),
...
with torch.no_grad():
    ...
    fait_model = functs._C._create_function_from_graph(
        "forward", fait_model.graph)
    ...
    functs.utils.evaluate_task(
        tracing_task, "dynamo+inductor", run_duration=2.)
