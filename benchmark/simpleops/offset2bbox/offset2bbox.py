...
class Offset2bboxTs(torch.nn.Module):
...
class Offset2bboxFuncTs(torch.nn.Module):
    def forward(self, boxes: torch.Tensor, offset: torch.Tensor):
        """
        Forward transform that maps proposal boxes to predicted ground-truth
        boxes using bounding-box regression deltas(offset). See bbox_transform_inv
...
eager_fn = Offset2bboxTs()
jit_fn = torch.jit.script(Offset2bboxTs())
functs_fn = functs.jit.script(Offset2bboxFuncTs())
