...
<<<<<<< HEAD
    ...
    cv_jit_model = [torch.jit.freeze(torch.jit.script(model))
                    for model in cv_model]
    cv_dynamo_model = [torch.compile(model, dynamic=True)
                       for model in cv_model]
    cv_nvfuser_model = [torch.jit.freeze(
        torch.jit.script(model)) for model in cv_model]
    cv_functs_model = [functs.jit.build(functs.jit.script(
        model, backend="aot"), example_in) for model, example_in in zip(cv_model, example_in)]

    backbone_time = [1.54, 1.35, 3.41, 5.136]

    def task(fn, feats): return lambda idx: fn(*feats[idx % len(feats)])
...
>>>>>>> 45225190966ef74b4186581e40d32a6d0584a5e5
    ...
    functs_mode_latency = np.array([float(evaluate_task(task(model, feats), name="{}_functs".format(cv_model_name), run_duration=3.).avg(round_to="ms")) + backbone_time
                                    for model, cv_model_name, feats, backbone_time in zip(cv_functs_model, cv_model_name, feats, backbone_time)])
    dynamo_mode_latency = np.array([float(evaluate_task(task(model, feats), name="{}_dynamo".format(cv_model_name), run_duration=3.).avg(round_to="ms")) + backbone_time
                                    for model, cv_model_name, feats, backbone_time in zip(cv_dynamo_model, cv_model_name, feats, backbone_time)])

    torch._C._jit_set_nvfuser_enabled(True)
    nvfuser_mode_latency = np.array([float(evaluate_task(task(model, feats), name="{}_nvfuser".format(cv_model_name), run_duration=3.).avg(round_to="ms")) + backbone_time
...
>>>>>>> 45225190966ef74b4186581e40d32a6d0584a5e5
        ...
        seq2seq_encoder_output = torch.randn(
            SEQ2SEQ_MAX_LENGTH, BATCH_SIZE, HIDDEN_SIZE).cuda()
...
>>>>>>> 45225190966ef74b4186581e40d32a6d0584a5e5
        ...
        k[:, :, :ATTENTION_START_LEN, :] = torch.randn(
            BATCH_SIZE, ATTENTION_NUM_HEAD, ATTENTION_START_LEN, ATTENTION_SIZE_PER_HEAD, dtype=torch.float32, device='cuda')
        v = torch.zeros(BATCH_SIZE, ATTENTION_NUM_HEAD, SEQ_LEN, ATTENTION_SIZE_PER_HEAD,
                        dtype=torch.float32, device='cuda')
        v[:, :, :ATTENTION_START_LEN, :] = torch.randn(
            BATCH_SIZE, ATTENTION_NUM_HEAD, ATTENTION_START_LEN, ATTENTION_SIZE_PER_HEAD, dtype=torch.float32, device='cuda')
        return [x, k, v]

    inputs = [[torch.rand([SEQ_LEN, BATCH_SIZE, INPUT_SIZE]).cuda().float()],
...
<<<<<<< HEAD
    ...
    nlp_nvfuser_model = [torch.jit.freeze(
        torch.jit.script(model)) for model in nlp_model]
...
>>>>>>> 45225190966ef74b4186581e40d32a6d0584a5e5
    ...
    functs_mode_latency = np.array([float(evaluate_func(model, inputs, name="{}_functs".format(nlp_model_name), run_duration=3.).avg(round_to="ms"))
                                    for model, nlp_model_name, inputs in zip(nlp_functs_model, nlp_model_name, inputs)])
    dynamo_mode_latency = np.array([float(evaluate_func(model, inputs, name="{}_dynamo".format(nlp_model_name), run_duration=3.).avg(round_to="ms"))
                                    for model, nlp_model_name, inputs in zip(nlp_dynamo_model, nlp_model_name, inputs)])
    torch._C._jit_set_nvfuser_enabled(True)
    nvfuser_mode_latency = np.array([float(evaluate_func(model, inputs, name="{}_nvfuser".format(nlp_model_name), run_duration=3.).avg(round_to="ms"))
