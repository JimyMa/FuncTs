...
class HomoConvBuilder {
 public:
  HomoConvBuilder(const Node* node) {
 ...
 private:
  ...
  //   LONG_TAIL_ASSERT(weight_shape.isComplete(), "weight shape must be
  //   static"); out_channels = weight_shape.at(0).value(); in_channels =
  //   weight_shape.at(1).value(); auto kernel_h = weight_shape.at(2).value();
