...
#include <functs/csrc/jit/ir/type_hashing.h>
...
class AliasDbCopy {
public:
  TORCH_API explicit AliasDbCopy(std::shared_ptr<Graph> graphi,
                                 bool isFrozen = false,
                                 bool descendFunctionCalls = false);
  TORCH_API ~AliasDbCopy();
  ...
  TORCH_API bool mayContainAlias(const at::ArrayRef<Value *> a,
  ...
  TORCH_API bool
  safeToChangeAliasingRelationship(const at::ArrayRef<Value *> &a,
                                   const at::ArrayRef<Value *> &b) const;
...
private:
  ...
  ska::flat_hash_map<TypePtr, Element *, HashTypeCopy, EqualTypeCopy>
      wildcardIndex_;
  Element *getWildcard(const TypePtr &type) const;
  c10::optional<Element *> tryGetOrCreateWildcard(const TypePtr &type);
  void addContainedTypesToFreshElement(Element *container_elem,
                                       const AliasTypeSet &mut_types);
  void pointUnionTypeElementToAllContainedTypes(Element *container_elem,
                                                const AliasTypeSet &mut_types);
...
// the right thing.
// TORCH_API void Lint(const AliasDbCopy *db);
