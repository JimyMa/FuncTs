...
#define REQ(cond) \
  ...
  bool canMerge(Node* consumer, Node* producer) {
    ...
    // Alias checks
    REQ(aliasDbCopy_->couldMoveBeforeTopologically(producer, consumer));
...
#undef REQ
  ...
  c10::optional<Node*> tryMerge(Node* fusion_group, Node* to_merge) {
    ...
    for (auto n : nodes_to_merge) {
      ...
      if (!aliasDbCopy_->moveBeforeTopologicallyValid(n, move_point)) {
  ...
  void createFusionGroups(Block* block) {
    ...
    // // Try to merge adjacent fusion groups together. Because we have only
    // merged
    ...
    //   ",
    //                        prev_fusion_group);
    //   if (auto merged_fusion_group = tryMerge(prev_fusion_group,
    //   fusion_group))
    //   {
