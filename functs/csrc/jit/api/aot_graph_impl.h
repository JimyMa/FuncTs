...
namespace torch {
namespace jit {
struct TORCH_API AotGraphFunction : public Function {
  // NOLINTNEXTLINE(cppcoreguidelines-pro-type-member-init)
  AotGraphFunction(c10::QualifiedName name, std::shared_ptr<Graph> graph,
                   std::function<void(AotGraphFunction &)> function_creator,
                   c10::optional<ExecutorExecutionMode>
                       executor_execution_mode = c10::nullopt)
      : name_(std::move(name)), graph_(std::move(graph)),
        executor_execution_mode_(executor_execution_mode),
        function_creator_(std::move(function_creator)) {}

  bool isGraphFunction() const override { return true; }

  void run(Stack &stack) override;

  std::function<void(AotGraphFunction &)> function_creator() const {
    return function_creator_;
  }

  c10::intrusive_ptr<c10::ivalue::Future>
  runAsync(Stack &stack, TaskLauncher taskLauncher = at::launch) override;
  ...
  const c10::QualifiedName &qualname() const override { return name_; }
  ...
  // will not affect executor if there is an existing executor
  // created for this function
  void _set_ignore_amp(bool ignore_amp) { force_no_amp_ = ignore_amp; }

  // if this isn't yet defined, run its method_creator function
  void ensure_defined() override;

  size_t num_inputs() const override { return graph()->inputs().size(); }
  ...
  void check_single_output() {
    TORCH_CHECK(graph()->outputs().size() == 1,
                "Method (but not graphs in general) require a single output. "
  ...
  using Function::call;
  bool call(Stack &stack, c10::optional<size_t> bailOut,
            c10::function_ref<void(const Code &)> f) override {
    f(get_executor().getPlanFor(stack, bailOut).code);
    return true;
  }

  void clear_optimized_graphs() { optimized_graphs_.fill(c10::nullopt); }
...
private:
  ...
  mutable std::array<c10::optional<std::shared_ptr<Graph>>,
  ...
  std::function<void(AotGraphFunction &)> function_creator_;
...
TORCH_API AotGraphFunction *tryToAotGraphFunction(Function &) noexcept;
TORCH_API AotGraphFunction &toAotGraphFunction(Function &);
TORCH_API const AotGraphFunction &toAotGraphFunction(const Function &);
