...
// A AotCompilationUnit is a list of named Functions
// with helper methods to iterate the list or invoke the function.
// Classes have a AotCompilationUnit holding the class methods,
// and Modules have a AotCompilationUnit holding the Functions that
// are used to implement their Methods

struct TORCH_API AotCompilationUnit {
  enum class FunctionType { Method, Hook, PreHook };
  // constructor that takes a set of functions to compile using the native
  // resolver
  explicit AotCompilationUnit(const std::string& source);
  AotCompilationUnit() = default;

  AotCompilationUnit& operator=(AotCompilationUnit&&) = default;
  AotCompilationUnit(AotCompilationUnit&&) = default;
  AotCompilationUnit& operator=(const AotCompilationUnit&) = delete;
  AotCompilationUnit(const AotCompilationUnit&) = delete;
  ...
  void set_optimized(bool o) {
    TORCH_WARN(
        "AotCompilationUnit::set_optimized() is deprecated and has no effect. "
        "Please use setGraphExecutorOptimize()");
  }

  bool is_optimized() const {
    TORCH_WARN(
        "AotCompilationUnit::is_optimized() is deprecated and always returns true. "
  ...
  Function* create_function(
    ...
    auto fn = torch::make_unique<AotGraphFunction>(
  ...
  // AotCompilationUnit. In Python, sometimes functions won't have unique qualified
...
// Functions.
struct AotStrongFunctionPtr {
  AotStrongFunctionPtr(std::shared_ptr<AotCompilationUnit> cu, Function* function)
      : cu_(std::move(cu)), function_(function) {
    TORCH_INTERNAL_ASSERT(cu_);
    TORCH_INTERNAL_ASSERT(function_);
  }
  std::shared_ptr<AotCompilationUnit> cu_;
  Function* function_;
};
...
using AotCompilationUnit = ::torch::jit::AotCompilationUnit;
} // namespace script
} // namespace jit
} // namespace torch
