...
void markNonParLoops(
  ...
  for (auto node : block->nodes()) {
    ...
    if (isLoop && node->inputs().size() > 2)
      nonParLoops.insert(node);
...
void ParallelizeLoops(const std::shared_ptr<Graph>& graph) {
  ...
  for (auto loop : loops)
    convertLoopToMap(loop, graph.get());
}
...
static Node* splitAt(
  ...
  moveNodesToBlock(
      splitNode, prevBlock->return_node(), nextBlock, valueMap, &refinedTypes);

  // Add return values to next block
  for (auto ret : nextRets)
    nextBlock->registerOutput(valueMap.at(ret));

  return nextParMap;
}

static void splitParallelMap(
    Node* curParMap,
    Graph* graph,
    ValueTypeMap& refinedTypes) {
...
void ConvertInfusibleMapsToLoops(
    const std::shared_ptr<Graph>& graph,
    ValueTypeMap& refinedTypes) {
  ...
  traversePostOrder(graph->block(), [&](Node* node) {
    ...
    if (frontNode->kind() == prim::FusionGroup)
      return true;
  ...
  for (auto parMap : infusibleMaps)
    convertMapToLoop(parMap, refinedTypes);
...
static bool isSimpleMap(Node* parMap) {
  ...
  if (parMap->input(0)->node()->kind() != prim::Constant)
    return false;
...
void UnrollSimpleMaps(
  ...
  // Unroll simple maps
  for (auto parMap : simpleMaps)
    unrollMap(parMap, refinedTypes);
}
