...
static c10::optional<Stack> tryRunNodes(Node* node) {
  ...
  if (isAliasing(node)) {
    ...
    if (std::any_of(
            output->uses().begin(), output->uses().end(), [](const Use& use) {
  ...
  switch (node->kind()) {
    ...
    case prim::ListConstruct: {
      listConstruct(
          stack,
          node->output()->type()->expectRef<ListType>(),
          node->inputs().size());
    } break;

    default: {
      auto schema = node->maybeSchema();
      if (!schema || schema->is_vararg())
        return c10::nullopt;
      try {
...
bool FoldConstantsTSSA(const std::shared_ptr<Graph>& graph) {
  ...
  rewrite(graph->block(), [&](Node* node) -> Node* {
    ...
    if (node->kind() == prim::If)
      return foldIfCond(node, changed);
    ...
    for (auto i = 0u; i < (*outputs).size(); i++) {
      ...
      if (!cnstVal)
        continue;
      if (ival.isNone())
        (*cnstVal)->setType(node->outputs()[i]->type());
